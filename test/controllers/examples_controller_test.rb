...
class ExamplesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create example" do
    assert_difference('Example.count') do
      post examples_url, params: { example: { content: @example.content, title: @example.title } }
    end
  ...
  test "should update example" do
    patch example_url(@example), params: { example: { content: @example.content, title: @example.title } }
